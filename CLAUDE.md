...
> - **⏳ Amplify's console still showed the domain as `AWAITING_APP_CNAME` at 19:30 while the domain was already serving with its Amazon certificate.** AWS says verification can lag up to ~30 min. If it is still not `AVAILABLE` next session, check `aws amplify get-domain-association --app-id dvewl3gkeo718 --domain-name glowpt.app`; the site is served either way.
