...
| AI reflections | Claude Haiku 4.5 on **Bedrock**, via the `glowpt-ai-response` Lambda, which assumes a role in the MANAGEMENT account because glowpt-prod's Bedrock quota is 0.0 |
...
- **AI reflections:** patient text goes to the `glowpt-ai-response` Lambda, which **assumes `arn:aws:iam::456112636877:role/GlowptBedrockInvoke` and calls Bedrock in the MANAGEMENT account** (glowpt-prod's Haiku quota is 0.0; the management account's is not). Deliberately outside the VPC: no DB, and it needs the Bedrock and STS endpoints. The role lives in `infra/bedrock/`, not the CDK stack, which cannot reach another account.
...
**✅ THE AI GAP IS CLOSED (2026-09-15).** The reflection runs on **Bedrock**, covered by the org-level AWS BAA. `api.anthropic.com` is gone from the running app. **⛔ Never add an Anthropic fallback for a Bedrock outage** — a failure that sends PHI outside the BAA would look exactly like success. The Lambda degrades to one fixed line of house copy, and a test enforces it. The superseded Supabase Team and Resend path is in `docs/history.md`.
...
> **It is now ONE item, not two. Bedrock closed 2026-09-15.**
> 1. **⚖️ THE ATTORNEY REVIEW.** Both drafts are written, committed and marked DRAFT — NOT FOR EXECUTION. **He has it in hand; do not chase (see STANDING RULES).** The three questions, in priority order: does the liability cap reach PHI claims ($4,200 is trivial, and a cap set too low can be struck out entirely) · is the clinical disclaimer enforceable and must it also appear in the patient UI · click-through or signature. **🆕 One cheap question to add: IS A GLOWPT CHECK-IN PART OF THE CLINIC'S DESIGNATED RECORD SET?** It decides whether deletion on request is easy or whether state retention rules bite.
>    - **✅ THE SUBCONTRACTOR-TABLE HAZARD IS GONE.** The BAA's table names AWS alone, which was false while the app called `api.anthropic.com` and became TRUE on 2026-09-15. The table is now correct as written.
>
> ### ✅ BEDROCK IS DONE (2026-09-15). DO NOT REOPEN IT.
> **The reflection runs on Claude Haiku 4.5 on Bedrock**, verified end to end through the live Lambda. **⛔ Do not file quota requests, reopen case `178761116000010`, chase AWS Activate, contact Sales for an Account Manager, or re-propose an Anthropic 1P BAA.** All of it is moot.
> - **🪤 THE LESSON, AND IT COST FIVE WEEKS: MEASURE THE ACCOUNT BEFORE ARGUING WITH AWS.** Every Haiku quota in `glowpt-prod` is **literally 0.0**, so there was never a number to negotiate up; the management account `456112636877` had **27M tokens/day** the entire time and had never been tested. One `aws service-quotas list-service-quotas --service-code bedrock` in each account would have ended it in a minute.
> - **The last step was not a quota at all:** Anthropic requires a **one-time use case form**, submitted ONCE at the organization's management account (Bedrock console → Model catalog → Claude Haiku 4.5 → Submit use case details). Submitted 2026-09-15; the model answered about a minute later.
...
> 1. **A new clinic is CLOSED until David opens `/admin` and presses Switch On, and SWITCH ON IS LOCKED UNTIL THE BAA IS RECORDED (2026-09-15).** The only human decision left in the flow. Recording the BAA asks to confirm first, the record then stays on screen as `BAA signed <date> · Clear`, and a mistaken one is cleared through `admin_clear_baa` — **which refuses while the clinic is ON, so switch it off first.**
> 2. **🔒 EVERY clinic is invite only, always. There is no switch.** Walk-in sign-up and the printable QR were removed 2026-09-05, and **the page's dead form was deleted 2026-09-18**: `/join/<slug>` is now one sentence, "You'll need an invite," kept for old links and printed QRs. **The `open_signup` column, `join_clinic` and its API route are KEPT so the feature can return** (David's 09-05 call); returning it means writing a form again, not removing a gate.
>
...
- **A new clinic is CLOSED until David presses Switch On in `/admin`, and ⛔ SWITCH ON NOW REQUIRES `baa_signed_at` (David, 2026-09-15): *"I shouldn't otherwise be able to switch on a clinic if the 'record baa signed' isn't ticked, right."*** Switching a clinic on is the moment real PHI may flow into it. Still two columns, `activated_at` (the gate) and `baa_signed_at` (the legal record), but no longer independent. **Switching OFF is deliberately ungated** — a clinic must be closeable in a hurry. The gate is enforced in the DATABASE, in `admin_set_clinic_active`, `join_clinic` and the `checkins` RLS policies — gating only the join would let an already-attached patient keep writing PHI to a switched-off clinic. **⚠️ The three existing clinics were switched on BEFORE this rule and carry no BAA record; the check fires when the switch is thrown, not continuously, so they keep running. Switch one off and it needs a BAA to come back.**
- **🗂️ A CLINIC HAS A LIFECYCLE (2026-09-16): Archive → Export → Delete**, the patient roster's shape one level up. Archiving switches it OFF; **restoring does not switch it back on.** Deleting works only from archived, takes the clinic's people and their logins, and asks for the name to be typed. **⛔ A PLATFORM ADMIN IS NEVER DELETED, only detached — David administers GlowPT with the address that manages Riverside PT.** `clinic_deletions` is the ledger; **nothing in it may identify a person.**
...
- **⚠️ A FOLLOW-UP ABOUT SOMETHING JUST CHANGED MUST NOT UNDO THE ORIGINAL ASK.** On 09-15 he asked for the invite's first-name box to say it could be edited; on 09-16 he asked for its label to match "Last Name", and the fix deleted the hint. He caught it on 09-18. **Re-read the first request before shipping the second.**
...
- **▶ `db/tests/owner_tests.sql` holds the assertions `glowpt_app` CANNOT make** (`users`, `clinic_deletions`: no grant at all). Put one in an app-role suite and it does not fail — **the run stops early and looks fine.**
...
- **BOTH NAMES ARE REQUIRED FOR EVERY USER, staff included (David, 2026-09-15): *"We need to be able to identify patients and staff and any users by first and last name."*** Enforced in the DATABASE, not only in the forms, in `invite_patient`, `invite_staff`, `join_clinic` and `rename_patient`. The earlier staff exemption (they are not on the patient roster) was too narrow: a clinic has as many Sarahs on the care team as in the caseload. **The COLUMNS stay nullable** so rows created before that date keep working, and `register_user` is deliberately untouched, because an invited person's names arrive from the invite row a moment after it runs. **⛔ David does not want a staff rename tool. He said so; do not offer it again.**
...
- **The card is NAVY with cream copy and NO frame, and that is settled.** It was white with a 3px frame for a few hours on 2026-09-14; David saw both and chose navy. *"I didn't like the big blue border anyway. The big issues we fixed the font size and the ladder nonsense."* **⛔ Do not re-propose a light card.**
- **⚠️ GMAIL'S iOS APP INVERTS THIS EMAIL AND IGNORES THE `color-scheme` META** (proven on David's phone both ways that evening). **David has accepted the flip: *"I know the iphone will flip it to white in dark mode, that's fine."* It is not a bug to chase.**
- **🎨 THE ONE COLOR RULE, and the whole yield of that round trip: Gmail flips LIGHTNESS, so a design is safe in both directions exactly when no color sits near its own background's lightness.** It killed a pale gray frame and a bright-amber-on-white wordmark in one evening. **On the navy card the bright `#FBC02D` is correct for the PT and for the clinic email's "may need attention" signal; on a light card both would have to go dark (`#B0730A` was the value that worked).** The constants carry that warning.
...
- **⛔ DECLARE A NEW COLUMN LAST IN ITS TABLE.** `add column` only appends, so one declared mid-table makes a migrated database differ from a fresh build forever. Only the rehearsal diff catches it.
...
- **`scripts/check-title-case.mjs` catches title-case stragglers** (a short string with no terminal punctuation is a LABEL, so every word after the first is capitalised unless AP leaves it alone). It found **20** on 2026-09-16 after David found two by eye. **⛔ IT IS NOT WIRED INTO THE BUILD AND MUST NOT BE — David, asked directly: *"NO, don't block deploys. Crikey. It can stay a check we run from time to time."*** Run it by hand; add a deliberate exception to `ALLOWED` in the script, with its reason. **⛔ It cannot see the emails in `infra/lambda/` (template literals); those are checked by eye.**
...
- **🗂️ CLINICS CAN BE ARCHIVED, EXPORTED AND DELETED (2026-09-16, `d14d0c9`).** David's ask, built as the patient roster's shape one level up; the JSON export was his call to build now, not shelve. Rules above. Tests **92 → 107**. **⚠️ The rehearsal caught a mid-table column and five assertions that could not fail; opening the screen caught a crash.**

- **🧑 BOTH NAMES FOR EVERY USER, AND THE BAA NOW UNLOCKS THE SWITCH (2026-09-15, `81613a0` → `9f85d49`).** Two things David found by walking a brand new clinic end to end. **Names:** the staff exemption on last names was too narrow — *"Clinics have too many patients and staff with same names so both should always be required."* Now enforced in `invite_staff`, `invite_patient`, `join_clinic` and `rename_patient`; columns stay nullable so older rows keep working. **The BAA:** *"I've been using my switched on clinics without ever clicking that."* `admin_set_clinic_active` requires `baa_signed_at` to switch ON (never to switch off), recording asks first, and `admin_clear_baa` corrects a mistake. Rules under STANDING RULES. Tests **84 → 92**.
  - **⚠️ A REHEARSAL AND AN RLS TRAP, both caught before shipping.** The names patch first produced a database differing from a fresh build by ONE COMMENT inside `invite_patient`. And three new tests "failed" a working feature because they read `public.clinics` and `access_log` **as `glowpt_app`, which RLS hides from a caller who is not in that clinic** — assert on the function's RETURN VALUE there, not on a table read.
  - **⚠️ The slug came off the `/admin` card** (only David ever saw it, and nothing user-facing uses it since walk-in joining was removed), and the invite screen's pre-filled first-name box finally got a label. **⛔ David does not want a staff rename tool, and walk-in joining is not to be re-proposed.**
...
- **🚫 AWS CLOSED THE BEDROCK CASE WITH A NO, AND DAVID APPLIED TO AWS ACTIVATE THE SAME MORNING (2026-09-07).** AWS resolved case `178761116000010` saying the Bedrock team prioritises **Activate members or accounts with a dedicated Account Manager**; everyone else waits for a "standard rollout" with no date. It never mentions the internal review promised on 09-03. **The Free Tier clause in it does not apply — the org is on the PAID plan, verified.**
  - **✅ Activate applied (Founders tier), linked to glowpt-prod `463556655381`, NOT the management account** — the Bedrock wall is in glowpt-prod and both credits and the association sit at account level. **⛔ REJECTED 2026-09-14.** Seen by David on the status page 09-15; **stages 1 and 2 passed, it was refused at stage 3, the decision itself. No reason given and no email had arrived** at `david@franklinaisolutions.com` (Outlook, not Gmail). **▶ STATUS PAGE: https://aws.amazon.com/startups/credits/status/ → AWS Credits → "AWS Activate Credits applications" → Reviewed tab.** Signs in with the **AWS BUILDER ID — `david@franklinaisolutions.com`, in David's password manager, a THIRD credential after root and SSO. NOT the Identity Center login**, which shows an empty account rather than an error.
