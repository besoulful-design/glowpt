...
> ## 🧭 NEW THREAD? READ THIS FIRST — CURRENT STATE ONLY (Sunday 2026-09-13, 18:50)
>
...
> - **✅ PREP DONE 2026-09-13 18:25 (`scripts/dns-prep.sh`, run by David):** Route 53 zone **`Z00899942MO7OU6SOCKAS`** in glowpt-prod holds an exact copy of the Netlify zone (verified record by record against the live answers). Nameservers for GoDaddy at cutover: `ns-809.awsdns-37.net` · `ns-1883.awsdns-43.co.uk` · `ns-29.awsdns-03.com` · `ns-1090.awsdns-08.org`. Amplify domain association created (apex + www → main), **certificate ISSUED 18:38** (David added the validation CNAME in Netlify DNS; status is now `AWAITING_APP_CNAME`, which means "proven, point the domain at me"). The Netlify zone turned out to hold **12** records, not 9: three dead Resend-era ones (`send` MX/SPF, `resend._domainkey`) were copied too so the switch is a true no-op; delete them once Netlify DNS is gone. Route 53 = SOA + NS + those 12, verified one by one against live answers.
> - **⏳ NOT DONE: THE CUTOVER ITSELF.** glowpt.app is still served by Netlify. **The runbook is `scripts/dns-cutover.sh` (read its header).** In order: (1) David changes the four nameservers at GoDaddy, site keeps serving from Netlify via the copied records, wait for `dig NS glowpt.app` to show awsdns; (2) `bash scripts/dns-cutover.sh amplify` flips apex + www to an ALIAS on `d1zcgq2clp38j4.cloudfront.net`, Amplify goes AVAILABLE; (3) David requests a sign-in code and it arrives; (4) rollback is `bash scripts/dns-cutover.sh netlify`. `status` mode is read-only and safe any time. Then the verification table is re-run against `https://glowpt.app`, and this doc gets its full regeneration.
> - **🪤 A name you dig BEFORE it exists is negatively cached by your resolver for the zone's SOA minimum (3600s here)**: my watcher said the validation CNAME was "not yet" for 20 minutes after Netlify was serving it. **Query the authoritative nameserver directly** (`dig @dns1.p04.nsone.net …` today, `dig @ns-809.awsdns-37.net …` after the switch) before concluding a record is missing. **DNS for glowpt.app is hosted on NETLIFY DNS (nsone.net), NOT GoDaddy** — GoDaddy is registrar only. Plan: Route 53 zone in glowpt-prod with an exact copy of today's records (inventory below), Amplify domain association, cert issued, THEN David changes the four nameservers at GoDaddy, then apex/www flip to Amplify. **The SES records live in that Netlify zone and MUST be recreated first or sign-in codes stop:** 3 DKIM CNAMEs (`<token>._domainkey` → `<token>.dkim.amazonses.com`; tokens from `aws sesv2 get-email-identity --email-identity glowpt.app`), `bounce.glowpt.app` MX `10 feedback-smtp.us-east-1.amazonses.com` + TXT `v=spf1 include:amazonses.com ~all`, `_dmarc` TXT `v=DMARC1; p=none;`. Apex A today = Netlify's `98.84.224.111` / `18.208.88.157`.
