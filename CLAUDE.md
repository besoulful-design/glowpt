...
> 1. **⛔ BEDROCK — AWS CLOSED THE CASE WITH A NO (2026-09-07).** Case `178761116000010` resolved: the Bedrock team expedites only **AWS Activate members or accounts with a dedicated Account Manager**; everyone else waits for a "standard rollout" with no date. **David applied to AWS Activate (Founders tier) the same morning, linked to glowpt-prod `463556655381`.** **⛔ REJECTED 2026-09-14.** Seen by David on the status page 09-15; **stages 1 and 2 passed, it was refused at stage 3, the decision itself. No reason given and no email had arrived** at `david@franklinaisolutions.com` (Outlook, not Gmail). **▶ STATUS PAGE: https://aws.amazon.com/startups/credits/status/ → AWS Credits → "AWS Activate Credits applications" → Reviewed tab.** Signs in with the **AWS BUILDER ID — `david@franklinaisolutions.com`, in David's password manager, a THIRD credential after root and SSO. NOT the Identity Center login**, which shows an empty account rather than an error. **🚨 BUT MEASURE BEFORE ARGUING: ON 2026-09-15 THE WALL WAS FOUND TO BE `glowpt-prod` ALONE.** Every Haiku 4.5 quota in `glowpt-prod` reads **literally 0.0** (tokens/day, tokens/minute, requests/minute), which is why the case went nowhere: there was no small number to negotiate. **The MANAGEMENT account `FranklinAI` `456112636877` is fully provisioned — 13.5M max tokens/day, 27M global tokens/day, 5M tokens/minute, 10 requests/minute — and had never been tested.** A test call there returns "your account is currently being verified, normally under 2 hours" (first Bedrock use; `aws-verification@amazon.com` if it persists), so **the quota is confirmed but a successful invocation is NOT yet proven.** The org-level AWS BAA covers the management account, so PHI there is as covered as anywhere. **▶ NEXT: retest the invoke in `franklinai-mgmt`; if it works, `glowpt-ai-response` assumes a role in the management account and calls Bedrock cross-account.** ⚠️ Running a workload in the management account is against AWS's own guidance and is a pragmatic unblock, reversible when glowpt-prod's quota opens. **⛔ Activate was rejected 09-14.** The other lane AWS named in the same breath is **an account with a dedicated Account Manager**, which comes through **AWS Sales** and costs nothing; that is the next free lever, with the "If NO" feedback link as a second. **⚠️ AND THE TWO PRODUCT-LEVEL OPTIONS ARE NOW LIVE AND ARE DAVID'S DECISION, NOT CLAUDE'S: ship with the AI reflection OFF, or de-identify the prompt so no PHI reaches Anthropic and the business-associate problem disappears.** They were parked precisely until Activate failed. It has. Raise them as a decision; do not act on either.
